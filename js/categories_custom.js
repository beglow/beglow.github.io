			...
			filterButton.on('click', function () {
				$('.product-grid').isotope({
					filter: function () {
						...
						var priceRange = $('#amount').val();
						var priceMin = parseInt(priceRange.split('-')[0].replace(/[^\d]/g, ''));
						var priceMax = parseInt(priceRange.split('-')[1].replace(/[^\d]/g, ''));
						var itemPriceText = $(this).find('.product_price').clone().children().remove().end().text();
...
function initPriceSlider() {
	$("#slider-range").slider({
		range: true,
		min: 0,
		max: 1200000, // bạn có thể chỉnh mức cao nhất ở đây
		step: 50000,
		values: [0, 650000],
		slide: function (event, ui) {
			$("#amount").val(formatPrice(ui.values[0]) + " - " + formatPrice(ui.values[1]));
		}
	});

	// Gán giá trị ban đầu cho ô hiển thị
	var val0 = $("#slider-range").slider("values", 0);
	var val1 = $("#slider-range").slider("values", 1);
	$("#amount").val(formatPrice(val0) + " - " + formatPrice(val1));
}
